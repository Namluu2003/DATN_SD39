...
import com.poly.rivo.infrastructure.vnpay.VNPayConfig;
...
public class VnpayController {
    ...
    @PostMapping("/refund")
    public ResponseObject refund(String transactionId, String amount, String orderInfo) {
        try {
            String vnp_Version = "2.1.0";
            String vnp_Command = "refund";
            String vnp_TmnCode = VNPayConfig.vnp_TmnCode;
            String vnp_IpAddr = "127.0.0.1"; // Replace with actual client IP if needed
            ...
            vnp_Params.put("vnp_TransactionNo", transactionId); // Use the original transaction ID
            ...
            String refundUrl = VNPayConfig.vnp_PayUrl + "?" + queryUrl;
            return new ResponseObject(refundUrl); // You may want to actually call the URL or process the response

        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            return new ResponseObject("Error creating refund request: " + e.getMessage());
        }
    }
}
