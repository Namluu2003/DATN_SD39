...
import { Link } from "react-router-dom";
import { Timeline, TimelineEvent } from "@mailtop/horizontal-timeline";
import { EditOutlined, FileOutlined, TruckOutlined, CheckCircleOutlined } from "@ant-design/icons";
...
import FormatDate from "~/utils/FormatDate";
import FormatCurrency from "~/utils/FormatCurrency";
import "./timeline.css"; // Giữ nguyên file CSS nếu có, hoặc chỉnh sửa nếu cần
import InfoBill from "./InfoBill";
import PaymentMethod from "./PaymentMethod";
import BillHistory from "./BillHistory";
import {
...
const BillDetail = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "images",
      render: (images, record) => (
        <div style={{ position: "relative", width: "120px", height: "120px", borderRadius: "8px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
          {record.discountValue !== null && (
            <div
              style={{
                position: "absolute",
    ...
    },
    {
      ...
      key: "name",
      render: (name, record) => (
        <div className="d-flex flex-column gap-1 p-3 bg-white rounded shadow-sm">
          {/* Tên sản phẩm */}
          <div className="fw-bold fs-4 text-primary">{name}</div>

          {/* Mã sản phẩm */}
          <div className="text-muted fs-5">
            <strong className="me-2">Mã:</strong>
            <span className="text-dark">{record.shoeCode}</span>
          </div>

          {/* Thông tin chi tiết */}
          <div className="text-muted fs-5 d-flex flex-wrap gap-4">
            <div>
              <strong>Xuất xứ:</strong> {record.xuatXu || "N/A"}
            </div>
            <div>
  ...
  return (
    <BaseUI>
      <div className="brand-container p-4 bg-light rounded shadow">
        <h6 className="fw-bold brand-title text-primary">Lịch sử hóa đơn</h6>
        <div className="my-4">
          <Timeline minEvents={8} placeholder maxEvents={billHistory.length}>
            {billHistory.map((item, index) => (
              <TimelineEvent
        ...
        <PaymentMethod
          ...
          onRefund={() => handleStatusChange(true, true)} // Truyền hàm xử lý hoàn tiền
        />
        <div className="d-flex align-items-center mt-5 bg-white p-3 rounded shadow-sm">
          <Title
            level={5}
            className="text-primary text-uppercase p-0 m-0 flex-grow-1"
          >
            Danh sách sản phẩm
          </Title>
          {(bill.status === 2 || (bill.status !== 9 && bill?.status <= 3)) &&
        ...
        <Table
          ...
          showHeader={true}
          rowClassName={(record) =>
            record.status === true ? "bg-light-danger" : "hover-row"
          }
          pagination={{
            current: currentPage,
            pageSize: pageSize,
